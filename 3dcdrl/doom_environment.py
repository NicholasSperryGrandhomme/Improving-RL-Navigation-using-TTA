...
class DoomEnvironment():
    ...
    def _create_game(self, params, idx, is_train, get_extra_info=False):
        ...
        #print(params)
        game = DoomGame()
        FULL_PATH = "/content/drive/My Drive/EPFL/Visual Intelligence/'Stupid Bitchass '/3dcdrl/"
        ...
        if params.scenario in VALID_SCENARIOS:
            ...
            if not is_train and params.test_scenario_dir:
                ...
                #print('loading file', filename)
                game.load_config(filename)
        elif params.scenario in VALID_TTA_SCENARIOS:
            filename = params.scenario_dir + params.scenario
            print(filename)
            game.load_config(filename)       
        else:
